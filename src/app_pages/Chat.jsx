...
export default function Chat() {
    ...
    // Pick up scan results passed from ScanScreen via sessionStorage
    useEffect(() => {
    ...
    const toggleIncognito = () => {
        if (!isIncognito) {
            ...
            setMessages([{
                id: uuidv4(),
                role: 'system-notice',
                content: 'This session is private. No messages will be stored.',
            }]);
        } else {
    ...
    const handleOpenFilePicker = () => {
        // Try dropzone first, fallback to native input
        try {
            openFilePicker();
        } catch {
    ...
    return (
        <div className={`${styles.chatPage} ${isIncognito ? styles.incognito : ''}`} {...getRootProps()}>
            ...
            <header className={styles.topBar}>
                ...
                <div className={styles.topBarActions}>
                    <motion.button
                        ...
                        whileTap={{ scale: 0.92 }}
                        title={isIncognito ? 'Exit private mode' : 'Enter private mode'}
                    >
                        {isIncognito ? (
                        ...
                        {isIncognito && <span className={styles.incogLabel}>Private</span>}
                    </motion.button>
            ...
            <AnimatePresence>
                {isIncognito && (
                    <motion.div className={styles.incognitoBanner} initial={{ height: 0, opacity: 0 }} animate={{ height: 40, opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.22 }}>
                        ...
                        <span>🕵️ Private session — nothing will be saved</span>
                                    ...
                                    <div className={styles.quickReplies}>
                                        {msg.suggestedActions?.length > 0 ?
                                            msg.suggestedActions.map(action => (
                                                <motion.button key={action} className={styles.actionChip} style={{ background: 'rgba(0, 201, 167, 0.15)', color: '#00C9A7', borderColor: '#00C9A7' }} onClick={() => navigate('/symptoms')} whileTap={{ scale: 0.95 }}>
                                                    {action} ➔
                                                </motion.button>
                                            ))
            ...
            <div className={styles.composer}>
                ...
                <motion.button className={styles.composerIcon} whileTap={{ scale: 0.96 }} title="Voice message (coming soon)">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
